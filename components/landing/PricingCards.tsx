...
import type { PricingSettings } from "@/actions/site-settings";
...
export default function PricingCards({ settings }: { settings?: PricingSettings }) {
  const pricing = settings ?? FALLBACK_PRICING;
  const freePlan = pricing.plans[0] ?? FALLBACK_PRICING.plans[0];
  const proPlan = pricing.plans[1] ?? FALLBACK_PRICING.plans[1];
  return (
    <section id="pricing" className="py-20 bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      <div className="container mx-auto px-4">
          ...
          <Card className="relative border-2 border-gray-200 hover:border-blue-300 transition-all duration-300 hover:shadow-lg">
            <CardHeader className="text-center pb-8">
              ...
              <div className="mt-4">
                ...
                <span className="text-gray-600 ml-2">{freePlan.priceSubLabel}</span>
              </div>
          ...
          <Card className="relative border-2 border-blue-500 hover:border-blue-600 transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1">
            <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
              <div className="bg-blue-500 text-white px-4 py-1 rounded-full text-sm font-semibold flex items-center">
                ...
                {proPlan.highlightBadge ?? "Most Popular"}
              </div>
            </div>
            <CardHeader className="text-center pb-8 pt-8">
              ...
              <div className="mt-4">
                ...
                <span className="text-gray-600 ml-2">{proPlan.priceSubLabel}</span>
              </div>
              <p className="text-gray-600 mt-2">
                {proPlan.description}
              </p>
        ...
        <div className="text-center mt-12">
          <p className="text-gray-600">{pricing.note}</p>
        </div>
      </div>
    </section>
  );
